...
async function main() {
  ...
  await Promise.allSettled(files.map(async (file) => {
    ...
    if (fs.statSync(file).size > 5 * 1024)
      warn('Unusually large file size');
    if (test(svg, '//@*[(starts-with(name(), "data-") or starts-with(name(), "class-"))]'))
      warn('Unnecessary data or class attribute');
    if (test(svg, '//@width | //@height'))
      warn('Use viewBox instead of height/width');
    if (test(svg, '/*/@id'))
      warn('Unnecessary id attribute in root element');
    ...
    if (test(svg, '//*[@version or @fill-rule or @script or @a or @clipPath or @class]'))
